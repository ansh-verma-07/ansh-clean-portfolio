...
const resolveRouteLabel = (path: string | null) => {
	if (!path) return '';
	if (path.startsWith('/projects/') && path !== '/projects') {
		const slug = path.split('/').filter(Boolean).pop();
		return slug ? formatSlugLabel(slug) : 'Project';
	}
	return routes[path] ?? 'Portfolio';
};
...
export default function Curve({ children, backgroundColor }: CurveProps) {
	...
	const activeLabel = router === '/' ? greetings[currentGreeting] : resolveRouteLabel(router);
